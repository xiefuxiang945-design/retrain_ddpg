...
class DDPGEnvironment(gym.Env):
    ...
    """def step(self, action, state, k):
        ...
        if self.vlm_features is not None and not self.vlm_fused_ready:
            # 每一步都生成模拟 VLM 距离（ISAC + 高斯噪声）
            isac_array = np.linalg.norm(loc_uav_array[:2] - loc_gt_array[:, :2], axis=1)
            vlm_array = isac_array + np.random.normal(loc=0, scale=5.0, size=isac_array.shape)  # 模拟视觉误差
        ...
        elif self.vlm_features is not None and self.vlm_fused_ready:
            # 后续直接用融合后的 VLM 距离
            vlm_dists = np.array(self.vlm_features).reshape(-1)
            for i in range(self.num_ground_stations):
                distances_UAV_GT_horizontal[i] = vlm_dists[i]
                r_kn += bandwidth[i] * self.communication(loc_uav_array[2], vlm_dists[i])
            fusion_debug = {
                "fused": vlm_dists.copy(),
                "g": np.ones((6, 1)),  # 占位
                "vlm": vlm_dists.copy(),
                "isac": np.zeros((6, 1))  # 占位
            }
        # 计算真实 GT 距离，作为监督目标
    ...
    def step(self, action, state, k, mode=0):
        flag = False
        r_kn = 0
        ...
        for i in range(loc_gt_array.shape[0
            ...
            if mode == 0:  # ISAC 测距
                ...
                distances_UAV_GT_horizontal[i] = d_true + np.random.normal(0, 4)
            elif mode == 1:  # VLM 测距
                d_true = np.linalg.norm(loc_uav_array[:2] - loc_gt_array[i, :2])
                distances_UAV_GT_horizontal[i] = d_true + np.random.normal(0, 5)
            elif mode == 2:  # Fusion，每步实时计算
                d_isac = np.linalg.norm(loc_uav_array[:2] - loc_gt_array[i, :2])
                d_vlm = d_isac + np.random.normal(0, 5)

                if getattr(self, "gate_module", None) is None:
                    self.gate_module = GateMechanism(feature_dim=1)

                _gate_device = next(self.gate_module.parameters()).device
                vlm_tensor = torch.tensor([[d_vlm]], dtype=torch.float32, device=_gate_device)
                isac_tensor = torch.tensor([[d_isac]], dtype=torch.float32, device=_gate_device)

                fused_tensor, _ = self.gate_module(vlm_tensor, isac_tensor)
                distances_UAV_GT_horizontal[i] = fused_tensor.detach().cpu().item()
    ...
    def baseline(self, state, mode=0):
        action = [-1 for _ in range(self.num_ground_stations + 3)]
        uav_x = state[0] * self.ground_length
        uav_y = state[1] * self.ground_length
        loc_uav_array = np.array([uav_x, uav_y, 100])
        ...
        GT_location_x = state[3:3 + self.num_ground_stations] * self.ground_length
        GT_location_y = state[3 + self.num_ground_stations:3 + 2 * self.num_ground_stations] * self.ground_length
        loc_gt_array = np.column_stack((GT_location_x, GT_location_y, np.zeros(self.num_ground_stations)))

        distances_UAV_GT_horizontal = np.zeros(self.num_ground_stations)

        for i in range(self.num_ground_stations):
            # 计算测距（和 step 中的 noise/模式一致）
            d_true = np.linalg.norm(loc_uav_array[:2] - loc_gt_array[i, :2])
            if mode == 0:  # ISAC
                d_meas = d_true + np.random.normal(0, 4)
            elif mode == 1:  # VLM
                d_meas = d_true + np.random.normal(0, 5)
            elif mode == 2:  # Fusion（用 gate + 监督训练）
                d_isac = d_true + np.random.normal(0, 1)  # ISAC 测距
                d_vlm = d_true + np.random.normal(0, 5)  # VLM 测距

                # 初始化 gate_module 和优化器（只做一次）
                if getattr(self, "gate_module", None) is None:
                    self.gate_module = GateMechanism(feature_dim=1)
                    self.gate_optimizer = torch.optim.Adam(self.gate_module.parameters(), lr=1e-3)
                    self.gate_loss_fn = torch.nn.MSELoss()

                # 确保 device 一致
                _gate_device = next(self.gate_module.parameters()).device
                vlm_tensor = torch.tensor([[d_vlm]], dtype=torch.float32, device=_gate_device)
                isac_tensor = torch.tensor([[d_isac]], dtype=torch.float32, device=_gate_device)
                gt_tensor = torch.tensor([[d_true]], dtype=torch.float32, device=_gate_device)
                ...
                # 取融合后的值作为测距结果
                d_meas = fused_tensor.detach().cpu().item()

            else:
                d_meas = d_true

            distances_UAV_GT_horizontal[i] = d_meas
        ...
        # 防护：若没有找到（理论上不会），退回默认行为
        if nearest_gs_index == -1:
            nearest_gs_index = 0

        # 计算朝向最近 GS 的 angle 并输出 action（与原来行为保持一致）
