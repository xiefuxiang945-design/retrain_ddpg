...
import os
import random
from Test_env_ddpg import DDPGEnvironment
...
### >>> VLM ADD: 新增缓存
REWARD_BUFFER_vlm = np.empty(shape=num_episodes)
Data_rate_BUFFER_vlm = np.empty(shape=num_episodes)
Throughput_BUFFER_vlm = np.empty(shape=num_episodes)
Energy_efficiency_BUFFER_vlm = np.empty(shape=num_episodes)

UAV_trajectory_x_vlm = np.zeros((1, num_step))
UAV_trajectory_y_vlm = np.zeros((1, num_step))
UAV_trajectory_z_vlm = np.zeros((1, num_step))
GT_trajectory_x_vlm = np.zeros((1, num_step))
GT_trajectory_y_vlm = np.zeros((1, num_step))
GT_trajectory_z_vlm = np.zeros((1, num_step))
...
for episode_i in range(num_episodes):
    ...
    ### >>> VLM ADD
    episode_reward_vlm = 0
    episode_throughput_vlm = 0
    episode_data_rate_vlm = 0
    episode_Energy_efficiency_vlm = 0
    ...
    UAV_positions_x = [state[0]];
    UAV_positions_y = [state[1]];
    UAV_positions_z = [state[2]]
    GT_positions_x = [state[3:3 + num_ground_stations]]
    GT_positions_y = [state[3 + num_ground_stations:3 + 2 * num_ground_stations]]
    GT_positions_z = [state[3 + 2 * num_ground_stations:3 + 3 * num_ground_stations]]
    ...
    ### >>> VLM ADD
    UAV_positions_x_vlm = [state_vlm[0]];
    UAV_positions_y_vlm = [state_vlm[1]];
    UAV_positions_z_vlm = [state_vlm[2]]
    GT_positions_x_vlm = [state_vlm[3:3 + num_ground_stations]]
    GT_positions_y_vlm = [state_vlm[3 + num_ground_stations:3 + 2 * num_ground_stations]]
    GT_positions_z_vlm = [state_vlm[3 + 2 * num_ground_stations:3 + 3 * num_ground_stations]]
    ...
    for step_i in range(num_step):
        ...
        # ISAC
        action = actor_isac(torch.FloatTensor(state).unsqueeze(0).to(device)).detach().cpu().numpy()[0]
        ...
        # VLM
        action_vlm = actor_vlm(torch.FloatTensor(state_vlm).unsqueeze(0).to(device)).detach().cpu().numpy()[0]
        next_state_vlm, reward_vlm, done, throughput_step_vlm, data_rate_step_vlm, Energy_efficiency_step_vlm, _ = env.step(
            action_vlm, state_vlm, 1)
        GT_positions_x_vlm.append(state_vlm[3:3 + num_ground_stations])
        GT_positions_y_vlm.append(state_vlm[3 + num_ground_stations:3 + 2 * num_ground_stations])
        GT_positions_z_vlm.append(state_vlm[3 + 2 * num_ground_stations:3 + 3 * num_ground_stations])
        UAV_positions_x_vlm.append(state_vlm[0]);
        UAV_positions_y_vlm.append(state_vlm[1]);
        UAV_positions_z_vlm.append(state_vlm[2])
        state_vlm = next_state_vlm
        episode_reward_vlm += reward_vlm
        episode_throughput_vlm += throughput_step_vlm
        episode_data_rate_vlm += data_rate_step_vlm
        episode_Energy_efficiency_vlm += Energy_efficiency_step_vlm
    ...
    ### >>> VLM ADD
    REWARD_BUFFER_vlm[episode_i] = episode_reward_vlm
    Data_rate_BUFFER_vlm[episode_i] = episode_data_rate_vlm / num_step
    Throughput_BUFFER_vlm[episode_i] = episode_throughput_vlm / num_step
    Energy_efficiency_BUFFER_vlm[episode_i] = episode_Energy_efficiency_vlm / num_step
    ...
    if Max_reward < episode_reward_our:
        ...
        UAV_trajectory_x_vlm = UAV_positions_x_vlm;
        UAV_trajectory_y_vlm = UAV_positions_y_vlm;
        UAV_trajectory_z_vlm = UAV_positions_z_vlm
        GT_trajectory_x_vlm = GT_positions_x_vlm;
        GT_trajectory_y_vlm = GT_positions_y_vlm;
        GT_trajectory_z_vlm = GT_positions_z_vlm
    ...
    print(f"DDPG-VLM-Episode: {episode_i + 1}, Reward: {round(episode_reward_vlm, 2)}")
...
plt.plot(REWARD_BUFFER, color='r', linestyle='--', label='DDPG-ISAC')
plt.plot(REWARD_BUFFER_vlm, color='r', linestyle='--', label='DDPG-VLM')
plt.xlabel('Episode');
...
plt.close()
print('reward', np.mean(REWARD_BUFFER_our), np.mean(REWARD_BUFFER_noisac), np.mean(REWARD_BUFFER), np.mean(REWARD_BUFFER_vlm))

plt.clf()  # 清除当前图像
# 绘制 Data_rate_BUFFER 图
plt.plot(Data_rate_BUFFER_our, marker='o', label='DDPG-ISAC')
plt.plot(Data_rate_BUFFER_noisac, label='DDPG')
plt.plot(Data_rate_BUFFER, color='r', linestyle='--', label='DDPG-ISAC')
plt.plot(Data_rate_BUFFER_vlm, color='r', linestyle='--', label='DDPG-VLM')
plt.xlabel('Episode')
...
# plt.show()
plt.close()  # 关闭图像窗口
print('datarate', np.mean(Data_rate_BUFFER_our), np.mean(Data_rate_BUFFER_noisac), np.mean(Data_rate_BUFFER), np.mean(Data_rate_BUFFER_vlm))
...
# plt.show()
plt.close()  # 关闭图像窗口
print('Throughput_BUFFER', np.mean(Throughput_BUFFER_our), np.mean(Throughput_BUFFER_noisac),np.mean(Throughput_BUFFER),np.mean(Throughput_BUFFER_vlm))
...
plt.plot(Energy_efficiency_BUFFER, color='r', linestyle='--', label='DDPG-ISAC')
plt.plot(Energy_efficiency_BUFFER_vlm, color='r', linestyle='--', label='DDPG-VLM')
plt.xlabel('Episode')
plt.ylabel('Energy Efficiency (Kbps/J)')
plt.title('DDPG Energy Efficiency')
plt.legend(loc='upper left')
...
# plt.show()
plt.close()  # 关闭图像窗口
print('Energy_efficiency_BUFFER', np.mean(Energy_efficiency_BUFFER_our), np.mean(Energy_efficiency_BUFFER_noisac),np.mean(Energy_efficiency_BUFFER),np.mean(Energy_efficiency_BUFFER_vlm))
...
ax.plot([x * width for x in UAV_trajectory_x_noisac], [y * width for y in UAV_trajectory_y_noisac],
        [z * height for z in UAV_trajectory_z_noisac], label='DDPG-only UAV Flight Path', marker='.', color='red')
ax.plot([x * width for x in UAV_trajectory_x],
        [y * width for y in UAV_trajectory_y],
        [z * height for z in UAV_trajectory_z],
        label=' DDPG-ISAC UAV Flight Path', marker='.', color='cyan')
ax.plot([x * width for x in UAV_trajectory_x_vlm],
        [y * width for y in UAV_trajectory_y_vlm],
        [z * height for z in UAV_trajectory_z_vlm],
        label=' DDPG-VLM UAV Flight Path', marker='.', color='orange')
# 初始化存储位置列表的列表
...
ax2.plot([x * width for x in UAV_trajectory_x_vlm],
         [y * width for y in UAV_trajectory_y_vlm],
