...
class UAVEnv(gym.Env):
    ...
    def step(self, action, k):
        ...
        for gs in self.ground_stations:
            ...
            if (gs['position'][0] + walkspeed * self.t * math.cos(alpha) < self.ground_length) and (
                    gs['position'][0] + walkspeed * self.t * math.cos(alpha) > 0):
                gs['position'][0] += walkspeed * self.t * math.cos(alpha)
            if (gs['position'][1] + walkspeed * self.t * math.sin(alpha) < self.ground_width) and (
                    gs['position'][1] + walkspeed * self.t * math.sin(alpha) > 0):
        ...
        # 位置控制函数，更倾向于在内部飞行
        if ((self.loc_uav[0] + dx_uav < self.ground_length) and (self.loc_uav[0] + dx_uav >= 0)) and (
                (self.loc_uav[1] + dy_uav < self.ground_width) and (self.loc_uav[1] + dy_uav >= 0)) and (
                (self.loc_uav[2] + dz_uav < self.height_max) and (self.loc_uav[2] + dz_uav >= self.height_min)):
            r1 = 0
        else:
            r1 = -4
        ...
        for i in range(loc_gt_array.shape[0]):
            ...
            distances_UAV_GT_horizontal[i] = np.linalg.norm(
                loc_uav_array[:2] - loc_gt_array[i, :2] + gaussian_matrix)  # 假设loc_gt_array[i]包含x和y坐标
    ...
    def communication(self, h, d):
        ...
        if ((self.loc_uav[0] < self.ground_length) and (self.loc_uav[0] >= 0)) and (
                (self.loc_uav[1] < self.ground_width) and (self.loc_uav[1] >= 0)) and (
                (self.loc_uav[2] < self.height_max) and (self.loc_uav[2] >= self.height_min)):
            r_kn = r_kn
        else:
            r_kn = 0
        return r_kn
    ...
    def reset(self):  # ok
        ...
        # 958030,good
        gtx = [36.997030855955494, 53.19896748977524, 20.019889656911783, 13.257760934502294, 54.30245664199407,
               74.13939293385138]
        gty = [43.30032996341227, 92.92182147894647, 82.01102859458813, 59.79515818116455, 30.841857576617237,
               67.14503554078097]
        # 946308
